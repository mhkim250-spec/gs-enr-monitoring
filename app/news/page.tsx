...
export default function NewsPage(){
 ...
 <section className="news-portal"><div className="news-hero"><img src="/news-hero.png" alt="재생에너지와 디지털 뉴스룸"/><div><span>GS E&amp;R · ENERGY NEWS DESK</span><h1>뉴스</h1></div></div><div className="news-masthead news-tools"><label className="news-search"><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="뉴스 전체 검색"/></label><strong>{filtered.length}개의 뉴스</strong></div>
 ...
 <section className="publisher-grid">{groups.filter((group)=>active==="all"||group.key===active).map((group)=><article className="publisher-card" key={group.key} style={{"--source-color":group.color} as React.CSSProperties}><header><div className={`publisher-logo ${group.key}`}><img src={group.logoUrl} alt=""/><span>{group.name}</span></div><a href={group.url} target="_blank" rel="noreferrer">매체 홈 ↗</a></header><ol>{group.articles.filter((article)=>!needle||article.title.toLocaleLowerCase("ko").includes(needle)).slice(0,6).map((article,index)=><li key={article.id}><a href={article.url} target="_blank" rel="noreferrer"><b>{index+1}</b><span>{article.title}</span><em>↗</em></a></li>)}</ol></article>)}</section>
