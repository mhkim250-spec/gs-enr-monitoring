...
export default function NewsPage(){
 ...
 <section className="publisher-grid">{groups.filter((group)=>active==="all"||group.key===active).map((group)=><article className="publisher-card" key={group.key} style={{"--source-color":group.color} as React.CSSProperties}><header><div className={`publisher-logo ${group.key}`}><img src={group.logoUrl} alt=""/><span>{group.name}</span></div><a href={group.url} target="_blank" rel="noreferrer">매체 홈 ↗</a></header><ol>{group.articles.filter((article)=>!needle||`${article.title} ${(article.topics||[]).join(" ")}`.toLocaleLowerCase("ko").includes(needle)).slice(0,6).map((article,index)=><li className="news-rich-item" key={article.id}><a href={article.url} target="_blank" rel="noreferrer"><b className="news-rank">{index+1}</b><span><strong>{article.title}</strong><i>{(article.topics||["산업"]).slice(0,4).map(topic=>`#${topic}`).join(" ")}</i></span><u>↗</u></a></li>)}</ol></article>)}</section>
