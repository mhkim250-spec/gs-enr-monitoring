...
import {classifyText,parseDate} from "./intelligence";
...
export default function TodayDashboard(){
 ...
 const now=new Date();const today=dayKey(now);const sorted=useMemo(()=>events.map(event=>({...event,parsed:parseDate(event.date),intel:classifyText(event.title,event.source)})).filter(event=>event.parsed).sort((a,b)=>a.parsed!.getTime()-b.parsed!.getTime()),[events]);
 const todayEvents=sorted.filter(e=>dayKey(e.parsed!)===today);const weekEvents=sorted.filter(e=>e.parsed!.getTime()>=new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime()&&e.parsed!.getTime()<Date.now()+7*86400000);const priorityEvents=weekEvents.filter(e=>e.intel.score>=65).slice(0,5);const lastUpdated=statuses.reduce((v,s)=>Math.max(v,s.updated_at||s.updatedAt||0),0);
 ...
 <section className="brief-overview"><article><span>오늘 행사</span><strong>{todayEvents.length}</strong><small>건</small></article><article><span>7일 이내</span><strong>{weekEvents.length}</strong><small>건</small></article><article><span>GS E&amp;R 관심 일정</span><strong>{priorityEvents.length}</strong><small>건</small></article></section>
 ...
 <section className="dashboard-panel"><header><div><span>GS E&amp;R RELEVANCE</span><h2>우선 검토 일정</h2></div><a href="/events">전체 보기 →</a></header>{priorityEvents.map(event=><a className="brief-event" href={event.url} target="_blank" rel="noreferrer" key={event.id}><div><span>{event.source} · {event.intel.importance}</span><b>{event.title}</b><small>{event.date} · {event.intel.topics.slice(0,3).map(t=>`#${t}`).join(" ")}</small></div><em>{event.intel.score}<small>/100</small></em></a>)}</section></div>
 </section></main>;
}
