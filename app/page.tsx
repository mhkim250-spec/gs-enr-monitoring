...
export default function TodayDashboard(){
 ...
 const load=useCallback(async(refresh=false)=>{if(refresh)setRefreshing(true);const suffix=refresh?`?refresh=${Date.now()}`:"";const get=async(path:string)=>{try{const response=await fetch(path+suffix,{cache:"no-store"});return response.ok?await response.json():{};}catch{return {};}};const [assembly,korcham,kweia,climate,kpx,status]=await Promise.all([get("/api/events"),get("/api/korcham"),get("/api/kweia"),get("/api/climate-sources"),get("/api/kpx"),get("/api/source-status")]);setEvents([
 ...
 <div className="dashboard-columns"><section className="dashboard-panel"><header><div><span>TODAY</span><h2>오늘 확인할 행사</h2></div><a href="/summary">주간 일정 →</a></header>{loading?<p className="dashboard-empty">행사를 정리하고 있습니다.</p>:todayEvents.length?todayEvents.map(event=><a className="brief-event simple" href={event.url} target="_blank" rel="noreferrer" key={event.id}><div><span>{event.source}</span><b>{event.title}</b><small>{event.host||event.location||event.date}</small></div><u>↗</u></a>):<p className="dashboard-empty">오늘 예정된 행사가 없습니다. 다음 일정을 미리 검토해 보세요.</p>}</section>
