...
async function refresh(request:Request){
 const origin=new URL(request.url).origin;const stamp=Date.now();const paths=["/api/events","/api/korcham","/api/kweia","/api/climate-sources","/api/kpx","/api/mcee","/api/environment-committee","/api/news"];
